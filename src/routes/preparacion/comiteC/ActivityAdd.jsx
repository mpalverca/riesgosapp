...
export const DialogAdd = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (
        props.polyData &&
        Array.isArray(props.polyData) &&
        props.polyData.length > 0
      ) {
        // Si polyData ya es un array, asegurar el formato correcto
        if (
          Array.isArray(props.polyData[0]) &&
          Array.isArray(props.polyData[0][0])
        ) {
          // Ya está en formato [[[lat,lng]]]
          formattedPolygon = props.polyData;
        } else if (
          Array.isArray(props.polyData[0]) &&
          typeof props.polyData[0][0] === "number"
        ) {
  ...
  if (!props.comiteAdds || props.comiteAdds.trim() === "") {
    return (
      <Snackbar open={true} autoHideDuration={5000}>
        <Alert severity="warning">No existe información de comité</Alert>
      </Snackbar>
    );
  }
    ...
    return (
      <Snackbar open={true} autoHideDuration={5000}>
        <Alert severity="error">
          No se puede agregar marcador de información en el comité, ya que el
          usuario no pertenece al Comité
        </Alert>
      </Snackbar>
    );
  }
